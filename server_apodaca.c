...
void prompt_client_action(int socket, int result) {
    ...
    case 1:
        ...
        log_message("Username and password valid. What would you like to do next? (1) Upload (2) Download (3) Exit:\n");
        break;
    case 2: 
...
void handle_client_upload(int socket) {
    ...
    send(socket, "Please type the filename to upload:", 35,0);
    ...
    while ((bytes_received = recv(socket,buffer,sizeof(buffer),0)) > 0)
    {
        ...
        printf("1byte_received: %d\n", bytes_received);
    }

    if (bytes_received == 0) {
        printf("File uploading complete.\n");
        log_message("File upload complete.\n");
    } else {
...
void handle_client_download(int socket) {
    ...
    send(socket, "Please type the filename to download:", 36, 0);
    ...
    if (file == NULL) {
        ...
        log_message("Error opening file for reading\n");
        return;
    }
